...
type Props = {
  quiz: Quiz & { quizQuestions: Pick<Quizquestion, "id" | "question" | "answer">[] };
};
...
const OpenEnded = ({ quiz }: Props) => {
  ...
  const currentQuestion = React.useMemo(() => {
    return quiz.quizQuestions[questionIndex];
  }, [questionIndex, quiz.quizQuestions]);
  ...
  const { mutate: checkAnswer, isPending: isChecking } = useMutation({
    mutationFn: async () => {
      ...
      document.querySelectorAll("#user-blank-input").forEach((...input) => {
        filledAnswer = filledAnswer.replace("_____", input.values.prototype);
        input.values.prototype = "";
      });
  ...
  const handleNext = React.useCallback(() => {
    checkAnswer(undefined, {
      onSuccess: ({ percentageSimilar }) => {
        ...
        if (questionIndex === quiz.quizQuestions.length - 1) {
          endQuiz();
          setHasEnded(true);
          return;
        }
  ...
  }, [checkAnswer, questionIndex, toast, endQuiz, quiz.quizQuestions.length,]);
  React.useEffect(() => {
